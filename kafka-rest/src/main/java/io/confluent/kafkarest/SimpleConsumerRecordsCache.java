  ...
  public List<ConsumerRecord<byte[], byte[]>> pollRecords(Consumer<byte[], byte[]> assignedConsumer,
    ...
    if (maxCachesNum > 0) {
      ...
      return CachePerTopicPartition
            .pollRecordsWithoutCaching(assignedConsumer, topicName,
                    partitionId, offset, count, maxPollTime, time);
    }
  }
  ...
  */
  private static class CachePerTopicPartition {
    ...
    */
    ...
    public CachePerTopicPartition(final KafkaRestConfig config,
    ...
    */
    ...
    public synchronized List<ConsumerRecord<byte[], byte[]>> pollRecords(
            Consumer<byte[], byte[]> assignedConsumer,
            final long offset,
            final long count) {
      if (count > 0) {
    ...
    * @return
    */
    public static List<ConsumerRecord<byte[], byte[]>> pollRecordsWithoutCaching(
            Consumer<byte[], byte[]> assignedConsumer,
            final String topicName,
            final int partitionId,
            final long offset,
