...
public class SchemaRegistryEnabledRequestFilter implements ContainerRequestFilter {
  ...
  @Inject
  public SchemaRegistryEnabledRequestFilter(KafkaRestConfig config) {
    this.config = config;
  }

  @Override
  public void filter(ContainerRequestContext requestContext) {
    boolean schemaRegistryEnabled = config.getBoolean(SCHEMA_REGISTRY_ENABLE_CONFIG);
    if (!Boolean.TRUE.equals(schemaRegistryEnabled)) {
      ErrorMessage errorMessage =
          new ErrorMessage(
              NOT_SUPPORTED_API_BY_STREAMS_ERROR_CODE, NOT_SUPPORTED_API_BY_STREAMS_ERROR_MESSAGE);
      Response response = Response.status(422).entity(errorMessage).build();
